...
(function (root, factory) {
    ...
    HTML.prototype.render = function (callback) {
        ...
        this._marshaller = new HipieDDL.Marshaller().proxyMappings(this.proxyMappings());
        var context = this;
        if (this.ddlUrl()[0] === "[" || this.ddlUrl()[0] === "{") {
            this._marshaller.parse(this.ddlUrl(), function () {
                postParse();
            });
        } else {
            this._marshaller.url(this.ddlUrl(), function () {
                postParse();
            });
        }
        function postParse() {
            var dashboards = createGraphData(context._marshaller, context.databomb());
