//http://dygraphs.com/jsdoc/symbols/src/dygraph.js.html
/*
...
//TODO
//interactionModel
//highlightSeriesOpts // own set of publish params for this??? its an object with same params as below
//strokeWidth .. idk if its per axis too
//strokePattern per series .. idk if its per axis or base
//strokeBorderWidth
//strokeBorderColor
//pointSize
//plotter per series??? and general  def per series have no idea how to do that
//fillGraph // per series and general

//labelsDivStyles todo
//width
//height 
...
(function (root, factory) {
    ...
    function dyChart() {
        ...
        //this._axesList = ['x','y','y2']; // todo publish param
    ...
    //dyChart.prototype.axisLabelFormatter = function(axis, func) {
    //    if (func == undefined) { return typeof this._axisLabelFormatter[axis] !== null ? this._axisLabelFormatter[axis] : undefined };
    //    if (this._axisLabelFormatter === 'undefined') { this._axisLabelFormatter = {}; }
    //    this._axisLabelFormatter[axis] = func;
    //    
    //}
    ...
    dyChart.prototype.publish("axesList", ['x','y','y2'], "array", "List of series");
    
    dyChart.prototype.publish("color", [,,[]], "custom-html-color", "");
    dyChart.prototype.publish("colorValue", [,,[]], "custom-number", "");
    ...
    dyChart.prototype.publish("legend", "", "set", "Legend Options", ["", "always", "follow"]); // onmouseover? option
    dyChart.prototype.publish("showRoller", false, "boolean", "000");




    dyChart.prototype.publish("annotations", null, "object", "000"); //?? do we need
    dyChart.prototype.publish("selection", null, "object", "000"); //?? do we need
    dyChart.prototype.publish("visibility", [], "array", "000"); //?? do we need
    
    dyChart.prototype.publish("customOptions", null, "object", "000"); // dont forget to look at this

    // Axis
    dyChart.prototype.publish("axisLabelColor", [,{'x':"#000000",'y':"#000000",'y2':"#000000"},], "custom-html-color", "");
    dyChart.prototype.publish("axisLabelFontSize", [,{'x':14,'y':14,'y2':14},], "custom-number", "");

    dyChart.prototype.publish("axisLabelWidth", [,{x:60,y:50},], "custom-number", ""); 
    dyChart.prototype.publish("axisLineColor", [,{'x':"#000000",'y':"#000000",'y2':"#000000"},], "custom-html-color", "");
    dyChart.prototype.publish("axisLineWidth", [,{x:0.3,y:0.3},], "custom-number", "");
    dyChart.prototype.publish("axisTickSize", [,{x:3,y:3},], "custom-number", "");
    ...
    dyChart.prototype.publish("gridLineWidth", [,{'x':0.3,'y':0.3},], "custom-number", "");

    dyChart.prototype.publish("labelsUTC", false, "boolean", "");
    dyChart.prototype.publish("labelsKMB", [false,{'y':false,'y2':false},], "boolean", "");
    dyChart.prototype.publish("labelsKMG2", [false,{'y':false,'y2':false},], "boolean", "");
    
    dyChart.prototype.publish("logscale", false, "boolean", "");
    dyChart.prototype.publish("panEdgeFraction", null, "number", "");
    
    dyChart.prototype.publish("pixelsPerLabel", [,{'x':70,'y':30},], "custom-number", ""); // Default: 70 (x-axis) or 30 (y-axes)
    ...
    dyChart.prototype.publish("drawPoints", [false,{},{}], "custom-array", "");
    dyChart.prototype.publish("fillGraph", [false,,{}], "custom-array", "");

    
    // only works when drawpoints = true
    dyChart.prototype.publish("pointSize", [1,{},{}], "custom-array", "");
    ...
    dyChart.prototype.publish("stepPlot", [false,{},{}], "custom-array", "");
    dyChart.prototype.publish("strokeBorderColor", ["#FFFFFF",{},{}], "special-array", "");
    
    //dyChart.prototype.publish("strokeBorderWidth", null, "number", ""); // make an inject
    dyChart.prototype.publish("strokeBorderWidth", [null,{},{}], "custom-array", ""); // make an inject

    dyChart.prototype.publish("strokePattern", [[],{},{}], "custom-array", ""); // make an inject or blank array?
    dyChart.prototype.publish("strokeWidth", [1.0,{},{}], "custom-array", "");
    ...
    dyChart.prototype.publish("gridLineColor", ["#808080",{'x':"#808080",'y':"#808080"},], "custom-html-color", "");
    ...
    dyChart.prototype.publish("highlightCircleSize", [3,{},{}], "custom-array", "");
    ...
    dyChart.prototype.publish("interactionModel", null, "object", ""); // NO DOCUMENTATION ON THIS
    ...
    dyChart.prototype.setSeriesOptions = function (domNode, element) {
        
    }
    dyChart.prototype.setAxisOptions = function (domNode, element) {
    ...
    dyChart.prototype.getChartOptions = function () {
        ...
        var chartOptions = { 
            ...
            'highlightSeriesOpts': this.highlightSeriesOpts(), // remove from publish maybe???
        ...
        // TODO extract columns from data? 
        ...
        // Series
        var perSeriesParams = [
            "strokeWidth","drawPoints","pointSize","highlightCircleSize","strokePattern","strokeBorderWidth","strokeBorderColor",
            "stepPlot","color","colorValue"
        ];
        for (var i = 0, j = this._series.length; i < j; i++) {
             ...
             for (var axisIdx = 0; axisIdx < perSeriesParams.length; axisIdx++) {
                var val = typeof this['_'+perSeriesParams[axisIdx]][2][this._series[i]] !== 'undefined' ? this['_'+perSeriesParams[axisIdx]][2][this._series[i]] : this['__meta_'+perSeriesParams[axisIdx]].defaultValue[2].hasOwnProperty(this._series[i]) ? this['__meta_'+perSeriesParams[axisIdx]].defaultValue[2][this._series[i]] : null // the better way
        ...
        for (var i = 0, j = this._axesList.length; i < j; i++) {
             ...
             for (var axisIdx = 0; axisIdx < perAxisParams.length; axisIdx++) {
                var val = typeof this['_'+perAxisParams[axisIdx]][1][this._axesList[i]] !== 'undefined' ? this['_'+perAxisParams[axisIdx]][1][this._axesList[i]] : this['__meta_'+perAxisParams[axisIdx]].defaultValue[1].hasOwnProperty(this._axesList[i]) ? this['__meta_'+perAxisParams[axisIdx]].defaultValue[1][this._axesList[i]] : null // the better way      
                if (val === 'undefined' || val === null) {
                    continue;
                }
        ...
        // Func
        var perAxisFuncs = ["axisLabelFormatter","ticker"];
        for (var i = 0, j = this._axesList.length; i < j; i++) {
             chartOptions['axes'][this._axesList[i]] = {}; // init
    ...
    dyChart.prototype.testData2 = function() {
        ...
        this
            .columns(["parabola","line","another line","sine wave","sine wave2"])
            .series(["parabola","line",'another line',"sine wave","sine wave2"])
            .strokeWidth([,,{'parabola':0,'line':1,'sine wave':3,'sine wave2':3}])            
            .highlightCircleSize([,,{'parabola':6,'sine wave':10,'sine wave2':3}])
            .pointSize([,,{'parabola':4,'line':1.5}])
            .strokePattern([,,{'parabola':null,'line':Dygraph.DASHED_LINE,'another line':[25, 5],'sine wave':Dygraph.DOTTED_LINE,'sine wave2':Dygraph.DOT_DASH_LINE}])
            .strokeWidth([2,,{'line':1,'sine wave':3,'sine wave2':2}])
            .strokeBorderWidth([,,{'parabola':2,'line':2,'sine wave':2}])
            .drawPoints([,,{'parabola':true,'line':true}])
            .data(data);
        return this;    
    }
